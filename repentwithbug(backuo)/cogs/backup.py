...
class RestoreAllButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(f"✅ Restored all: recreated {recreated} channel(s) and reset {reset_count} active channel(s).", ephemeral=True)
...
class BackupRestoreView(discord.ui.View):
    ...
    def __init__(self, bot: commands.Bot, user: discord.Member, backup_id: str, deleted_channels: list, active_channels: list, all_roles: list, all_channels: list):
    ...
    async def reset_channel_settings(self, guild: discord.Guild, channel: discord.abc.GuildChannel, ch_data: dict):
...
class Backup(commands.Cog):
    ...
    class BackupGroup(app_commands.Group):
        ...
        async def list_backups(self, interaction: discord.Interaction):
            ...
            lines = []
            for b in backups[:15]:
                lines.append(f"**{b['name']}** — ID: `{b['backup_id']}` — *{b['created_at'][:19].replace('T', ' ')}*")
        ...
        @app_commands.command(name="restore", description="Interactively restore selected channels and roles from a backup (Admin/Owner only)")
        @app_commands.describe(backup_id="The ID of the backup snapshot to restore")
        async def restore(self, interaction: discord.Interaction, backup_id: str):
            ...
            backup = await get_backup(guild.id, backup_id)
            if not backup:
                return await interaction.followup.send(embed=error_embed(f"Backup with ID `{backup_id}` not found."))

            roles = await get_backup_roles(backup_id)
            channels = await get_backup_channels(backup_id)
            ...
            embed = discord.Embed(
                ...
                description="Use the menus and buttons below to selectively recreate missing channels, reset active channels' permissions and settings, or restore missing roles.",
                color=0x4488FF
            )
